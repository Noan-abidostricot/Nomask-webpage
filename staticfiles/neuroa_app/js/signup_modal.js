...
function updateSubmitButton() {
    ...
    window.submitButton.disabled = !(isEmailValid && isPasswordValid && window.isConditionsAccepted && isFirstNameFilled && isLastNameFilled);
}
...
function init() {
        ...
        if (window.firstNameInput.value.trim() !== '' && window.lastNameInput.value.trim() !== '' && validateEmail() && checkPassword() && window.signUpValidCheckbox.checked) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.querySelectorAll('.top-header').forEach(e => e.classList.add('hidden'));
    ...
    document.getElementById('resetModal').addEventListener('click', function(e) {
        e.preventDefault();
        document.querySelectorAll('.top-header').forEach(e => e.classList.remove('hidden'));
